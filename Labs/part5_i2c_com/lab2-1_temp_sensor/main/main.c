...
void app_main(void) {
    ...
    /* Install I2C driver : MASTER mode, GPIO SDA/SCL, SDA/SCL Pullup enable, Freq=10KHz */
	...
	for (;;) {
        ...
        result = lm75a_readRegister(raw); 
             
        printf("%x \t %x\t", raw[0], raw[1]);
        /* Compute temperature */
