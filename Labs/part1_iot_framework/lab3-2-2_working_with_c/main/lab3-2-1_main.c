...
void defaultPrintSensorValues(struct SensorSetStruct *sensorSet) {
  ...
  ESP_LOGW(TAG, "> %s = %d", sensorSet->pressure.name, sensorSet->pressure.value.i);
}
...
struct SensorSetStruct sensorSet = {
  ...
  while (1) {
    // Wait for 1 sec. 
    usleep(1000000);
    updateAndDisplaySensorValues(&sensorSet, defaultUpdateSensorValues, defaultPrintSensorValues);
  }
}
