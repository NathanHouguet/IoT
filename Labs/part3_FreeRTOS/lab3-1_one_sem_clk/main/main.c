...
SemaphoreHandle_t xSemClk = NULL;
...
void app_main(void) {
	...
	if(xSemClk == NULL){
	DISPLAY ("Failed to create the semaphore");
	}else{
	...
	xTaskCreatePinnedToCore(vTaskDecTable,	"Task DecTable", STACK_SIZE,	(void*)"DescTable", DEC_TABLE_TASK_PRIORITY, (void*) NULL,CORE_0);
